...
describe('auth (database)', { skip: !hasDatabase ? 'SONE_TEST_DATABASE_URL not set' : false }, () => {
  ...
  test('effectiveRole is pure and needs no database', () => {
    ...
    assert.equal(
      effectiveRole(claims, { id: 'child', workspaceId: 'ws', ancestorIds: ['root'], restrictedAt: null }),
      'commenter',
    );
    assert.equal(
      effectiveRole(claims, { id: 'elsewhere', workspaceId: 'ws', ancestorIds: [], restrictedAt: null }),
      null,
    );
  });
