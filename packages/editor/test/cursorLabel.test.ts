...
test('the caret label shows the display name', () => {
  const cursor = presenceCursor({ displayName: 'Markus', color: '#2563eb' });
...
test('a missing name reads as a person, not as nothing', () => {
  ...
  for (const state of [{}, { displayName: '' }, { displayName: 42 }]) {
...
test('a display name is inserted as text, never as markup', () => {
  ...
  const cursor = presenceCursor({ displayName: '<img src=x onerror=alert(1)>' });
...
test('the colour is used, and a missing one does not become "undefined"', () => {
  const coloured = presenceCursor({ displayName: 'A', color: '#ff0000' });
  assert.match(coloured.getAttribute('style') ?? '', /#ff0000/);

  const plain = presenceCursor({ displayName: 'A' });
