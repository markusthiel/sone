...
nimmt ein Passwort nur als `AUTH LOGIN` an, SONE hat unbesehen `AUTH PLAIN`
geschickt. Jetzt liest der Client, welche Verfahren das Relay nach dem
Verschlüsseln anbietet, und spricht LOGIN, wo es PLAIN nicht gibt. Bietet ein
...
Beispiel und Zitat färben den Hintergrund leicht in ihrer Farbe und zeigen
rechts im Kasten ein kräftiges Symbol — der Text bleibt, wie er ist, und lässt
sich weiterhin getrennt einfärben. Jede Art hat einen eigenen Eintrag im
`/`-Menü (eigene Gruppe „Callouts“; „hinweis“ tippen zeigt alle), und im
Block-Menü lässt sich die Art eines bestehenden Hinweises unter „Darstellung“
umstellen. Im Markdown-Export steht die Art als fette erste Zeile
(`> **Warning**`) und kommt beim Import so zurück (ADR-0188).

**Zitate haben eine Quelle.** Im Block-Menü eines Zitats gibt es ein Feld
...
was being written. They are on `ghcr.io/markusthiel/sone` and can be
